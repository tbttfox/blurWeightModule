    ...
    if ((theCommandIndex == ModifierCommands::LockVertices) ||
        ...
        if (this->paintMirror != 0) {
            ...
            if (status == MStatus::kFailure) {
                MGlobal::displayError(MString("Something went wrong. EXIT the brush and RESTART it")
                );
                return;
            }
        }
...
MStatus SkinBrushContext::getMesh()
{
    ...
    this->accelParams = meshFn.uniformGridParams(33, 33, 33); // I dont know why, but '33' seems to
                                                              // work well
...
MStatus SkinBrushContext::getTheOrigMeshForMirror()
{
    ...
    this->accelParamsOrigMesh = meshOrigFn.uniformGridParams(33, 33, 33); // I dont know why, but
                                                                          // '33' seems to work
                                                                          // well
...
void SkinBrushContext::getColorWithMirror(int vertexIndex, float valueBase, float valueMirror,
    ...
    if ((theCommandIndex == ModifierCommands::LockVertices) ||
        (theCommandIndex == ModifierCommands::UnlockVertices)) {
        if (theCommandIndex == ModifierCommands::LockVertices) { // lock verts if not already
                                                                 // locked
